...
- **Integrations**: `/api/integrations/*` exposes a static catalogue of 25 connectors with encrypted credentials (includes Gmail, Outlook, Twilio added in sprint).
- **Email Outreach**: Real Gmail (OAuth2) + Outlook (MSAL) providers in `services/comm/email.service.ts`. Auto-triage via Ollama (`/api/comm/triage`). Sequence editor UI at `components/operator/outreach-sequence-dialog.tsx`.
- **Desktop Screen Control**: `services/desktop-input.service.ts` gated by `DESKTOP_LIVE=1`. Uses `screenshot-desktop` + `@nut-tree-fork/nut-js` for real screen capture and input injection. Vision verification via `moondream:latest`.
...
- **Electron Desktop Shell (Task #211)**: `artifacts/omninity-desktop` wraps the omninity-website frontend and api-server in an Electron shell. Features: system tray icon (programmatic 16×16 RGBA or `assets/tray-icon.png`), close-to-tray behavior, embedded Express server (via `startServer()` export from api-server), contextBridge preload (`electronAPI.getApiPort()`), and electron-builder producing `.dmg` (macOS) and `.exe` (Windows) installers. Build: `pnpm --filter @workspace/omninity-desktop run build`. The Replit preview workflow for this artifact fails to detect the port (platform limitation for manually-registered artifacts not created via `createArtifact()`); the Electron app itself builds and runs correctly outside Replit.
...
- **Knowledge Base (Task #12)**: A local second-brain with deterministic FNV-1a hash-bucket TF-IDF embeddings, sentence-aware chunking, hybrid search, and URL ingest. Local embed logic extracted to `lib/kb-embed-local.ts` for sharing with the re-index service.
- **Local Embeddings & Vector Store (Task #241)**: Ollama embeddings (nomic-embed-text, local), OpenAI ada-002 (cloud), Qdrant (local, port 6333), ChromaDB (local, port 8000), Pinecone (cloud), Weaviate Cloud (cloud) backends. Pure embed logic in `lib/kb-embed-local.ts`; bridge in `services/kb-embeddings.service.ts`; re-index generator in `services/kb-reindex.service.ts`; SSE re-index route at `POST /api/knowledge/reindex`; status at `GET /api/knowledge/reindex/status`. The `vector-store` capability type is now registered alongside `embeddings` in the registry. Switching either backend in the UI shows a re-index prompt with SSE progress stream. SQLite fallback is always available when no vector store is configured. All fetch() calls privacy-logged.
...
- **Capability Runtime Switcher (Task #237)**: Extends the existing LLM runtime switcher to all six non-LLM AI capability types — ImageGen, WebSearch, TTS, Embeddings, VectorStore, and CodeSandbox. Core files: `services/capability/types.ts` (interfaces incl. `EmbeddingsRuntime.embed()` and `VectorStoreRuntime.upsert/search/delete`), `services/capability/registry.ts` (concrete adapters for embeddings + vector store, stubs for others), `services/capability.service.ts` (DB persistence via `runtimeCredentials` with `cap:` prefix), `routes/capabilities/index.ts` (REST endpoints: list, get, set-active, health, detect-local). DB schema at `lib/db/src/schema/capability-settings.ts`. Operator UI at `components/operator/capability-runtime-settings.tsx` — includes re-index prompt banner for embeddings/vector-store switches. All backends default to null; SQLite fallback for KB search. Tier-review compliant: fetch() calls privacy-logged, module-level Maps/Sets annotated.
- **Omninity OP Pitch Deck (omninity-pitch-deck)**: A 10-slide partner & customer pitch deck artifact (`artifacts/omninity-pitch-deck/`, slug: `omninity-pitch-deck`). Dark premium tech aesthetic (#070810 bg, #3B82F6 blue, #8B5CF6 violet), Syne + DM Sans fonts, all colors via CSS design tokens (`--slide-primary`, `--slide-accent`, `--slide-bg`, `--slide-text`, `--slide-muted` + RGB variants in `src/index.css`). Narrative arc: problem → solution → capabilities → differentiation → partnership CTA. Slides 5–8 use AI-generated UI mockup images (operator pages are auth-gated). Slide validate script at `scripts/validate-slides.ts`.
