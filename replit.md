...
2. `.local/tasks/OMNINITY_BUG_PREVENTION_STANDARDS.md` — Twelve mandatory bug prevention standards every task must implement. Covers contract-first development, Zod validation, per-task testing, tier review gates, DB transactions, idempotency keys, optimistic locking, circuit breakers, feature flags, the shared types package, performance budgets & benchmarks, and security patterns & sandboxing.
...
- `pnpm run tier-review` — run the automated 14-point tier review gate (run after every tier merges before activating the next; includes Standard 11 performance/bundle checks and Standard 12 security checks)
